<?php
        ...
        function removeGrad($gradStudentID){
            ...
            $query .= "WHERE id = %d";
            $query  = sprintf($query,
                              mysql_real_escape_string($gradStudentID));
		...
		function calculateSchoolCredit($oldRank, $newRank){
			...
			$query = sprintf($query,
							 mysql_real_escape_string($oldRank),
							 mysql_real_escape_string($newRank));
        ...
        function isGradListPaid2($gradListID){
        	...
        	$result = mysql_query($query) or die('Paypal response still pending...');
        ...
        function calculateGradFees($gradListID, $userID, $fullFee){
                        ...
                        $updateStatus = $this->updateGradDiplomaRequirement($gradListID, $diplomaCount);

                        if($updateStatus == false){
                //                die('An error has occurred while determining the diploma count...');
                        }
        ...
        function priceVerifyStart($gradListID){
            ...
            mysql_query($query) or die('Unable to start local price verification step...');
        ...
        function priceVerifyFinal($gradListID, $PVCValue){
            ...
            if (mysql_num_rows($result) != 0){
                ...
                $query = sprintf($query,
                                 mysql_real_escape_string($gradListID),
                                 mysql_real_escape_string($PVCValue));

                $result = mysql_query($query) or die('Unable to start local price verification step...');
        ...
        function insertPaypalTransaction($txn_id, $item_name, $item_number, $payment_date, $payment_amount){
        	...
        	$result = mysql_query($query) or die('Error at insertPaypalTransaction(...) in GradListDAO.php');
        }        
        ...
        function markListPaid($userID, $gradListID, $amount){
            ...
            mysql_query($query) or die('Unable to mark GradList as Paid due to ... ' . mysql_error());
        }
        ...
        function markListReadOnly($gradListID, $status = 1){
            ...
            if ($status == 1){
				...
				$query .= 'SET read_only = 0 ';
			}

            $query .= 'WHERE id = %d';
			$query = sprintf($query,
                             /*mysql_real_escape_string*/($gradListID));
            mysql_query($query) or die('Unable to mark graduation list as READ ONLY due to ... ' . mysql_error());
        }
