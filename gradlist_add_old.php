<?php
    ...
    if ($_SERVER['REQUEST_METHOD'] == 'POST'){
        ...
        if ($step == 2){
			...
			if (!isset($addToListButton2) && !isset($gotoNextStep)){
				$step = 2;
			}
			
			if (isset($addToListButton) || isset($addToListButton2)){
            	for($i = 0; $i < count($studentIDs); $i++){
                	$gradListDAO->addGrad($gradListID, $studentIDs[$i], ${oldRank_.$studentIDs[$i]}, ${newRank_.$studentIDs[$i]});
                }
            }
        }
